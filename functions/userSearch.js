...
module.exports.run = async (message, args, user, bot) => {
    ...
    do {
        ...
        else if (message.mentions.users.size > 0) { user = message.mentions.users.first(); }
        else if (/^(\d{17,21})$/.test(user)) { user = await Promise.resolve(message.bot.users.fetch(user)); }
        else if (message.bot.users.find("username", user) !== null) { user = message.bot.users.find("username", user); }

        message.channel.guild.members.forEach(element => {
            if (valid === false) {
    ...
    if (valid === false) { message.channel.send(text); data = text; }
